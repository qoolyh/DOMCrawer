...
var Element = function Element(values, children) {
    var i = 0;
    this.id = values[i++];
    this.pid = values[i++];
    this.w = values[i++];
...
Element.prototype.getSpecialTags = function () {
    var tmp = [this.search, this.footer, this.header, this.image, this.logo, this.navigation, this.is_input, this.is_txt, this.is_title];
    var str = '1';
...
function getBitMap(node) {
    var bitmap = [];
    for (var i = 0; i < node.w; i++) {
        bitmap[i] = [];
        for (var j = 0; j < node.h; j++) {
            bitmap[i][j] = 0;
        }
    }
    return bitmap;
...
function draw_bitMap(bitmap, node, ancestor) {
    var offset_left = node.x - ancestor.x;
    var offset_top = node.y - ancestor.y;
    for (var i = 0; i < node.w; i++) {
        for (var j = 0; j < node.h; j++) {
...
Tree.prototype.getCoverage = function (idx) {
    var node = this.elems[idx];
    var bitmap = getBitMap(this.elems[idx]);
    var descendant = this.getDescendant(this.elems[idx].id);
    for (var i = 0; i < descendant.length; i++) {
        draw_bitMap(bitmap, descendant[i], this.elems[idx]);
    }
    var one_number = 0;
    for (var _i = 0; _i < node.w; _i++) {
        for (var j = 0; j < node.h; j++) {
            one_number = bitmap[_i][j] === 1 ? one_number + 1 : one_number;
        }
    }
    var coverage = one_number / (node.w * node.h);
    if (isNaN(coverage)) {
        coverage = -1;
    }
    return coverage;
...
Tree.prototype.sortByLv = function (array) {
    ...
    (0, _Util.quickSort)(lv, zeros_idx);
    var tmp = [];
    for (var _i2 = 0; _i2 < zeros_idx.length; _i2++) {
        tmp[_i2] = array[zeros_idx[_i2]];
    }
    for (var _i3 = 0; _i3 < zeros_idx.length; _i3++) {
        array[_i3] = tmp[_i3];
    }
};
...
Tree.prototype.init = function () {
    ...
    this.sortByLv(zeros);
    for (var _i4 = 0; _i4 < zeros.length; _i4++) {
        this.deleteElemAt(zeros[_i4]);
    }
    this.updateNodesType();
    this.updateNodesCoverage();
};
Tree.prototype.deleteElemAt = function (id) {
    var node = this.elems[id]; // that's the problem, the index is not the id!
    var children = node.children;
    var idx = node.sib_order;
    if (node.pid === 0) {
        this.elems[node.id].isZero = 0;
        return;
    }
    var parent = this.elems[node.pid];
    for (var i = idx + 1; i < parent.children.length; i++) {
        //update node's siblings first
        var sib_idx = parent.children[i].idx;
        if (this.elems[sib_idx]) {
            var new_order = this.elems[sib_idx].sib_order + children.length - 1;
    ...
    for (var _i5 = 0; _i5 < children.length; _i5++) {
        ...
        var cld_idx = children[_i5].id;
        this.elems[cld_idx].sib_order += idx;
        this.elems[cld_idx].pid = node.pid;

        this.elems[cld_idx].lv -= 1;
        var _new_order = children[_i5].sib_order + idx;
        children[_i5].sib_order = _new_order;
        children[_i5].pid = node.pid;
        children[_i5].lv -= 1;
    }
...
Tree.prototype.getSingleBranch = function (leaf) {
    var single_branch = [];
    var current_idx = leaf;
    var p = this.elems[current_idx].pid;
    var children_num = this.elems[p].children_num;
...
Tree.prototype.updateNodesType = function () {
    var single_leaves = this.getSingleLeaves(this.getLeaves());
    for (var i = 0; i < single_leaves.length; i++) {
        var single_branch = this.getSingleBranch(single_leaves[i]);
        if (single_branch.length > 0) {
            var values = this.elems[single_branch[0]].getSpecialTags();
            var cross = values;
            for (var k = 0; k < single_branch.length; k++) {
                var current = this.elems[single_branch[k]].getSpecialTags();
                cross = parseInt(values, 2);
                current = parseInt(current, 2);
                cross = cross | current;
            }
            values = cross.toString(2);
            for (var _k = 0; _k < single_branch.length; _k++) {
                this.elems[single_branch[_k]].setSpecialTags(values);
            }
        }
    }
};
...
Tree.prototype.dfs_traverse = function (dbName) {
    var sql_array = [];
    var sql = "insert into `" + dbName + "` (`ID`, `parentID`, `width`, `height`, `offsetLeft`, `offsetTop`," + "`v_side`, `h_side`, `l_side`, `t_side`, `shapeAprnc`, `isZero`," + "`tag`, `color`, `fontColor`, `overflow`, `font_size`, `font_weight`," + "`word_count`, `word_len`, `search`, `footer`, `logo`, `image`," + "`navigation`, `bottom`, `top`, `fills_height`, `fills_width`, `selector`," + "`lv`, `sib_order`,`children_num`," + "`is_href`, `sib_num`,`textarea`,`is_input`,`is_txt`,`is_title`,`coverage`,`typeCls`) values" + "\r\n";

    sql_array.push(sql);
    for (var i = 1; i < this.elems.length; i++) {
        if (this.elems[i]) {
            var tmp_sql = "(" + this.elems[i].getProperties() + ")," + "\r\n";
            sql_array.push(tmp_sql);
        }
    }
    var last = sql_array[sql_array.length - 1];
...
Tree.prototype.getDescendant = function (root_idx) {
    var children = this.elems[root_idx].children.slice();
    var descendant = children.slice();
...
Tree.prototype.updateIframeOffset = function () {
    ...
    for (var _i7 = 0; _i7 < frames.length; _i7++) {
        var idx = frames[_i7];
        var px = this.elems[idx].x,
            py = this.elems[idx].y,
            pw = this.elems[idx].w,
            ph = this.elems[idx].h;
        var descendant = this.getDescendant(idx);
        for (var k = 0; k < descendant.length; k++) {
            var node = descendant[k],
...
Tree.prototype.checkOverlap = function () {
    ...
    for (var _i8 = 0; _i8 < overflow_root.length; _i8++) {
        var idx = overflow_root[_i8];
        var px = this.elems[idx].x,
            py = this.elems[idx].y,
            pw = this.elems[idx].w,
            ph = this.elems[idx].h;
        ...
        for (var k = 0; k < descendant.length; k++) {
            ...
            var cx = node.x,
                ...
                cw = node.w,
                ch = node.h;
            var is_In = isIn(cx, px, px + pw) && isIn(cx + cw, px, px + pw) && isIn(cy, py, py + ph) && isIn(cy + ch, py, py + ph);
            if (!is_In) {
...
function partition(data, array, left, right) {
    var key = data[array[left]];
    var array_key = array[left];
    var pivot = left;
    while (left < right) {
...
function qSort(data, array, left, right) {
    if (left < right) {
        var pivot = partition(data, array, left, right); // now the position of pivot is correct
        var msg = 'pivot=' + pivot + ' left=' + left + ' right=' + right;
...
var page_width = $(document.body)[0].clientWidth;
var page_height = $(document.body)[0].clientHeight;

var dbName = get_page_name();
if (dbName.indexOf("_sql") > 0) {
    dbName = dbName.substring(0, dbName.indexOf("_sql"));
}

var elems = [];
var tree = new _Tree.Tree(null, elems);
...
function main() {
    ...
    var lv = 0;
    var root = getElem(node, pid, lv, 0, 1);

    // sql_array.push(values);
    var children = getChild(node, root.id, lv + 1);
    ...
    // tree.deleteElemAt(2);
    tree.init();
    var sql_array = tree.dfs_traverse(dbName);
    var file = new File(sql_array, dbName + ".sql", { type: "text/plain;charset=utf-8" });
...
function getElem(node, pid, lv, sib_order, sib_num) {
    ...
    var x = parseInt($(node).offset().left);
    var y = parseInt($(node).offset().top);
    var children_num = $(node).children().length;
    var verticalSidedness = page_height === 0 ? 0 : (y + h) / page_height;
    verticalSidedness = verticalSidedness > 1 ? 1 : verticalSidedness;
    verticalSidedness = verticalSidedness.toFixed(2);
    var horizontalSidedness = page_width === 0 ? 0 : (x + w) / page_width;
    horizontalSidedness = horizontalSidedness > 1 ? 1 : horizontalSidedness;
    horizontalSidedness = horizontalSidedness.toFixed(2);
    var leftSidedness = (x / page_width).toFixed(2);
    var topSideness = (y / page_height).toFixed(2);
    var shapeAppearance = w !== 0 && h !== 0 ? Math.min(w / h, h / w) : 0;
    shapeAppearance = shapeAppearance.toFixed(2);
    var isZero = w * h === 0 ? 1 : 0;
    var tag = $(node).get(0).tagName.toLowerCase();
    var color = $(node).css("background-color");
    var bkimg = $(node).css("background-image");
    var fontColor = $(node).css("color");

    var pdt = parseInt($(node).css('padding-top'));

    var pdl = parseInt($(node).css('padding-left'));
    ...
    var word_len = wordSize(content);
    var m = content.replace(/(\n)+|(\r\n)+/g, "");
    m = m.replace(/(\s)+/g, " ");
    var attr = getAttr($(node));
    var search = isSearch(node, tag, attr) ? 1 : 0;
    var footer = contain_keyword(node, tag, attr, "footer") ? 1 : 0;
    var header = contain_keyword(node, tag, attr, "header") ? 1 : 0;

    var image = bkimg !== "none" || contain_keyword(node, tag, attr, "img") ? 1 : 0;
    var logo = contain_keyword(node, tag, attr, "logo") ? 1 : 0;
    var navigation = contain_keyword(node, tag, attr, "nav") ? 1 : 0;
    var bottom = y / page_height >= 0.9 ? 1 : 0;
    var top = y / page_height <= 0.1 ? 1 : 0;
    var fillsHeight = h / page_height >= 0.9 ? 1 : 0;
    var fillsWidth = w / page_width >= 0.9 ? 1 : 0;
    isZero = w < 0 || h < 0 || x < 0 || y < 0 ? 1 : isZero;
    x += pdl;
    y += pdt;

    var is_href = contain_keyword(node, tag, attr, "href") ? 1 : 0;
    var textarea = contain_keyword(node, tag, attr, "text") ? w * h : 0;
    var is_input = contain_keyword(node, tag, attr, "input") ? 1 : 0;
    var is_txt = content.length > 0 ? 1 : 0;
    var is_title = tag.charAt(0) === 'h' && tag.charAt(1) >= '1' && tag.charAt(1) <= '6' ? 1 : 0;
    ...
    var values = [gid, pid, w, h, x, y, children_num, isZero, overflow, search, footer, header, image, logo, navigation, is_href, textarea, is_input, is_txt, is_title, sib_num, tag, lv, verticalSidedness, horizontalSidedness, leftSidedness, topSideness, shapeAppearance, color, fontColor, fontSize, fontWeight, word_count, word_len, bottom, top, fillsHeight, fillsWidth, selector, sib_order, typeCls];
    var element = new _Element.Element(values, null);
    element.idx = gid;
    element.pidx = pid;
    return element;
}
...
function getAttr(node) {
    ...
    for (var i = 0; i < attr.length; i++) {
        var atrValue = attr.item(i).value;
...
function getText(node) {
    var textVec = node.contents().filter(function () {
    ...
    if (textVec.length === 1) {
        ...
        var _str = [];
        for (var i = 0; i < textVec.length; i++) {
            if (i === textVec.length - 1) {
                _str[i] = textVec[i].nodeValue;
            } else {
                _str[i] = textVec[i].nodeValue;
            }
        }
...
function getChild(node, id, lv) {
    ...
    var children = [];
    var sib_order = 0;
    var sib_num = child.length;
    $(child).each(function () {
        gid = gid + 1;
        var child_elem = getElem(this, id, lv, sib_order, sib_num);
        ++sib_order;
        var tag = $(this).get(0).tagName.toLowerCase();
        if (tag.indexOf("frame") > 0 || tag.indexOf("FRAME") > 0) {
            var innerElem = $(this).contents().find('html');
            var children_elem = getChild(innerElem, gid, lv + 1);
            child_elem.children = children_elem;
        } else {
            var _children_elem = getChild(this, gid, lv + 1);
            child_elem.children = _children_elem;
        }
...
function formattedCls(cls) {
    ...
    pieceCls = cls.split(/ +/);
    var formated = "";
    for (var i = 0; i < pieceCls.length; i++) {
...
function getContent(node) {
    ...
    for (var i = 0; i < text.length; i++) {
        var idx = html.indexOf(text[i]);
        var count = 0;
        if (idx !== prev) {
            var subHtml = html.substring(prev, idx);
            prev = idx + text[i].length - 1;
            var tag = subHtml.match(/<[a-z]+>|<\/[a-z]+>/g);
            var headStack = [];
            if (tag === null) {
...
function getPath(node) {
    ...
    for (var i = 0; i < path.length; i++) {
        ...
        if (i !== 0) {
            var idx = $(path[i - 1]).children(nd).index(path[i]);
            if (idx === 0) {
                prev = '>';
            } else {
                prev = '>';
...
},{"./Element.js":1,"./FileSaver.js":2,"./Tree.js":3,"./jquery.js":6}],6:[function(require,module,exports){
